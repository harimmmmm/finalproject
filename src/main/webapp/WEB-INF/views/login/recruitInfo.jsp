...
// 						            modalBody.append(introductionDiv);
						    ...
						    // 모든 체크박스에 대한 이벤트 핸들러를 제거한 후 다시 추가
// 							$('.resume-checkbox').off('change').on('change', function() {
// 							    // 체크된 체크박스의 rsmNo 값을 가져옴
// 							    var rsmNo = $(this).val();
							
// 							    // 다른 체크박스들의 체크를 해제
// 							    $('.resume-checkbox').not(this).prop('checked', false);
							
// 							    // 선택된 이력서의 소개를 보여주는 div 생성 또는 제거
// 							    if ($(this).is(':checked')) {
// 							        // 선택된 이력서의 소개를 보여주는 div 생성
// 							        // ...
// 							    } else {
...
// 							});
							...
							// 돋보기 아이콘 클릭 이벤트 핸들러
// 							$('.resume-view-icon').off('click').on('click', function() {
...
// 		                $('.site-button').on('click', function () {
		                $('.site-button').off('click').on('click', function () {
		                    // 선택한 이력서 정보를 담을 배열
		                    var selectedResumes = [];
		                    // 선택된 체크박스의 값을 배열에 추가
